...
async def remove_slot_date(message: Message, state: FSMContext, bot):
    ...
    db: Database = bot.db
    slots = await db.get_all_slots(slot_date)  # все слоты (доступные и нет)
    if not slots:
        await message.answer("На этот день нет слотов.")
        await state.clear()
        return

    # Сохраняем дату и показываем слоты для выбора
    await state.update_data(remove_date=date_str)
    await state.set_state(AdminStates.removing_slot_time)

    # Формируем клавиатуру со слотами (только доступные для удаления)
    builder = InlineKeyboardBuilder()
    for slot in slots:
        if slot['is_available']:  # только свободные слоты можно удалить
...
async def view_schedule(message: Message, state: FSMContext, bot):
    ...
    db: Database = bot.db
    # Сначала показываем все слоты с доступностью
